...
class TimestampLocator(ticker.Locator):
    ...
    def _process(self, vmin, vmax):
        ...
        xticks = map(lambda x: x + vmin, xticks) ##zoom in and out
        return xticks
    ...
    def infer_scale(self, dmin, dmax):
        ...
        numMinutes2 = numMinutes/15
        nums = [('AS', numYears), ('M', numMonths), ('W', numWeeks), ('D', numDays), ('H', numHours), ('15min', numMinutes2), ('1min', numMinutes)]
        byIndex = None
        for key, num in nums:
        ...
        return byIndex if byIndex else nums[-1][0]
...
class TimestampFormatter(object):
    ...
    def set_formatter(self, ax, mintick=10):
        self._locator = TimestampLocator(self.index, mintick)
...
class Grapher(object):
    ...
    def plot(self, name, series, index=None, fillna=None, secondary_y=False, 
        ...
        if is_datetime:
            self.mintick = kwargs.pop('mintick') if kwargs.has_key('mintick') else 15
    ...
    def setup_datetime(self, index):
        ...
        if self.formatter is None and self.skip_na and is_datetime:
            ...
            self.formatter.set_formatter(self.ax, self.mintick)
